...
#         print('w_n: {}'.format(w_n))
    ...
    def print_results(self, train_dataset, validation_dataset, threshold):
        ...
        plt.savefig(f'trained_models_pics_{self.cnn_name}.png', format = 'png')
    ...
    @staticmethod
    def get_best_thresholds(y_test, y_hat_test, labels, num_labels):

        threshold_list = np.arange(0.0, 1, 0.01)

        best_f1_label = np.zeros((num_labels))

        best_t_label = np.zeros((num_labels))
